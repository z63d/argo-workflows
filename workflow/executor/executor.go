...
func (we *WorkflowExecutor) reportResult(ctx context.Context, result wfv1.NodeResult) error {
	...
	return retryutil.OnError(wait.Backoff{
		...
		if apierr.IsForbidden(err) {
			log.WithError(err).Warn("failed to patch task set, falling back to legacy/insecure pod patch, see https://argo-workflows.readthedocs.io/en/v3.5.2/workflow-rbac/")
